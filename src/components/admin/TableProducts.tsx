    ...
    <div className='maiTabProducts'>
      <div className='content-actionsHeader-category'>
        ...
        >
          <option value='ALL'>
            {`${
              allCategories.isLoadingInfo ? "Chargement..." : "Toutes les catég."
            }`}
          </option>
      ...
      <ContentTable>
          ...
          {!allProducts.isLoadingInfo &&
            allProducts?.products?.map((item: any, key: number) => (
              <WidgetLgTrRows key={key}>
                ...
                <WidgetTd>{item.remise || "Non défini"}</WidgetTd>
        ...
        {allProducts.isLoadingInfo && <Grid container direction="column-reverse" alignItems="center" justifyContent="center" spacing={5}> <div>  <br /> <br /> <br /> <br />  </div> </Grid> }

        {allProducts.isLoadingInfo && <Grid container direction="column-reverse" alignItems="center" justifyContent="center" spacing={5}> <Loader /> </Grid> }
      </ContentTable>
  ...
  return (
    <>
      {allClients?.clients?.map((item: any) => (
        <>
          {item.id === client ? (
            <span>{`${item.name || "Inconnu"} ${item.lastname || "---"}`} </span>
          ) : (
            <span></span>
          )}
        </>
