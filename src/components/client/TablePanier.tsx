...
const TablePaniers = () => {
  ...
  const confirmCommand = async () => {
    ...
    if (createCommand?.command?.panier.length === 0) {
      showToast({
        message: "Veillez faire d'abord une commande. ",
        typeToast: "dark",
        autoClose: false
      });
      return history.push("/");
    ...
    try {
      ...
      const { data } = await postAPI("command/", { ...command });
      if (data.results.success) {
        dispatch({
          type: createCommandTypes.SET_CREATE_COMMAND
        });
        showToast({
          message: "Votre commande a été enregistrée avec succes ",
          typeToast: "success"
        });
      }
  ...
  return (
    <div className='maiTabProducts margin-20 '>
